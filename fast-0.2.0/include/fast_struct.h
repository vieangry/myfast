...
{
	u8  dmac[ETH_ALEN];	/* Ethernet source address. */
	u8  smac[ETH_ALEN];	/* Ethernet destination address. */
	...
	u8  frag:4,		/* One of OVS_FRAG_TYPE_*. */
		port:4;   /* Input Port*/
	//ip;
	...
	union {
		struct {			
			...
			union {
				struct {
					u16 sport;		/* TCP/UDP/SCTP source port. */
					u16 dport;		/* TCP/UDP/SCTP destination port. */
					u16 flags;		/* TCP flags. */
				} tp;
				struct {
		...
		struct {
			...
			union {
				...
				struct {					
					...
					struct in6_xaddr target;	/* ND target address. */
				} nd;
#endif
			};
		} ipv6;
...
/*-----------------------------------流表相关----------------------------------------------*/
/*--------------------------------863项目Lisp配置数据结构定义------------------------------ */
struct Lisp_port_config{
	u64 port_addr_low64;      
	u64 port_addr_high64;
	u64 port_mac;
	u64 port_host_mac;
}__attribute__((packed));

struct Lisp_rule_config{
	u64 dst_ip_low;
    u64 dst_ip_high;
    u64 mask_low;
    u64 mask_high;
...
struct um_metadata{
	...
	u64 ts:44,			/*报文接收的序号*/
		flowID:14,		/*流ID号*/
		priority:3,		/*报文优先级*/
		discard:1,		/*指示报文是否丢弃，默认为0，表示不丢弃，置1时表示丢弃*/
		pktdst:1,		/*报文的输出目的方向，0表示输出到网络端口，1表示输出到CPU*/
		pktsrc:1;		/*报文的输入源方向，0表示网络端口输入，1表示从CPU输入*/
	u64 outport:16,		/*报文输出端口号，bitmap形式*/
		seq:12,			/*报文接收时的序列号（每个端口独立维护一个编号）*/
		dstmid:8,		/*报文下次处理的目的模块编号*/
		srcmid:8,		/*报文上次处理时的模块编号*/
		len:12,			/*报文长度，最大表示4095字节*/
		inport:4,		/*输入端口号，最多表示16个输入端口*/
		ttl:4;			/*报文通过模块的TTL值，每过一个处理模块减1*/
