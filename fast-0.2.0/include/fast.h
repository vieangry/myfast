...
/*--------------weixin----------------*/
int weixin_packet_check(struct fast_packet *pkt,int pkt_len);

/*---------------863项目 Lisp 寄存器配置函数-----------------*/
int port_set(int port_num,struct Lisp_port_config date);
void init_rule_lisp(void);                   //初始化规则，默认action填0
int rule_set (int idx,int inport,struct Lisp_rule_config lisp_rule);
